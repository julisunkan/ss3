class PDFGenerator {
  constructor() {
    ...
    // Register the embedded font on PDF creation
    if (window.jspdf && window.jspdf.API && window.jspdf.API.events) {
      window.jspdf.API.events.push(['addFonts', () => {
        this.jsPDF.API.addFileToVFS("NotoSans-Regular.ttf", notoSansRegular);
        this.jsPDF.API.addFont("NotoSans-Regular.ttf", "NotoSans", "normal");
        this.jsPDF.API.addFont("NotoSans-Regular.ttf", "NotoSans", "bold");
      }]);
    }
  }
  ...
  // … generatePDFForPreview and printPDF follow same structure with doc.setFont
  ...
  addItemsTable(doc, documentData) {
    ...
    doc.setFont("NotoSans", "bold")
       .setFillColor(240).rect(15, startY, 180, 10, 'F');
  ...
  addTotals(doc, documentData) {
    ...
    let y = Math.max(220, 150 + 15 + documentData.items.length * 10 + 20);

    doc.text('Subtotal:', 140, y);
    doc.text(`${curr}${t.subtotal.toFixed(2)}`, 180, y);
    y += 10;
    doc.text(`Tax (${t.taxRate}%):`, 140, y);
    doc.text(`${curr}${t.taxAmount.toFixed(2)}`, 180, y);
    y += 10;
    doc.setFont("NotoSans", "bold").setFontSize(12).text('Total:', 140, y);
    doc.text(`${curr}${t.grandTotal.toFixed(2)}`, 180, y);
  }
  ...
  // add generatePDFForPreview / printPDF similarly using doc.setFont()
